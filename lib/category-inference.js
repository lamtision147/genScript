...
export function inferCategoryFromProductName(name = "") {
  ...
  const isPhoneCaseLike = has(/(op|ốp)\s*lung|lung\s*(dien\s*thoai|iphone|android|smartphone)|phone\s*case|case\s*(iphone|android|phone)|cover\s*(iphone|android|phone)|bao\s*da\s*(dien\s*thoai|iphone|android)/);
  if (isPhoneCaseLike) return "phoneTablet";
  ...
  if (has(/\bao\b|\bdam\b|dam\s*(midi|maxi|mini|body|suong)|\bdress\b|\bvay\b|chan\s*vay|so\s*mi|hoodie|\bquan\b|jean|denim|cardigan|blazer|croptop|sleepwear|pajama|pyjama|do\s*ngu|quan\s*ngu|shorts?/)) return "fashion";
  ...
  if (has(/yen\s*mach|granola|snack|do\s*uong|thuc\s*pham|food|an\s*kieng|ruou|vang\b|wine|beer|bia|whisky|whiskey|vodka|champagne|sparkling|soju|sake|cocktail/)) return "food";
