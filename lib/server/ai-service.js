...
import {
  inferCategoryFromProductName as inferCategoryFromNameShared,
...
const VISION_FORCE_DATA_URL = true;
...
const MIN_SUGGEST_IMAGE_DATA_URL_LENGTH = 1500;
const MAX_SUGGEST_IMAGE_PAYLOAD_COUNT = 4;
const MIN_STRONG_IMAGE_DATA_URL_LENGTH = 3000;
const SUGGEST_ADVANCED_FIELD_KEYS = [
  "usage",
  "skinConcern",
  "routineStep",
  "dimensions",
...
const SEARCH_FIRST_BRAND_CATALOG = [
  "Apple", "Samsung", "Xiaomi", "Oppo", "Vivo", "Sony", "LG", "Dell", "HP", "Asus", "Acer", "Lenovo", "MSI",
  "Logitech", "Razer", "JBL", "Anker", "Baseus", "Corsair", "ViewSonic", "AOC", "BenQ", "Philips", "Panasonic",
  "Guerlain", "Dior", "Chanel", "YSL", "Lancome", "Estee Lauder", "L'Oreal", "Vichy", "CeraVe", "Bioderma", "Nivea",
  "Nike", "Adidas", "Puma", "Uniqlo", "Zara", "H&M", "Converse", "New Balance"
];

const SEARCH_FIRST_OBJECT_HINTS = [
  { keywords: ["nuoc hoa", "perfume", "fragrance", "eau de parfum", "edp", "guerlain", "dior", "chanel", "ysl"], category: "fragrance", vi: "Nước hoa", en: "Perfume" },
  { keywords: ["serum", "cleanser", "toner", "skincare", "moisturizer", "kem duong", "sua rua mat"], category: "skincare", vi: "Sản phẩm chăm sóc da", en: "Skincare product" },
  { keywords: ["headphone", "earbud", "tai nghe", "airpod", "loa", "speaker"], category: "electronics", vi: "Tai nghe / thiết bị âm thanh", en: "Audio device" },
  { keywords: ["monitor", "man hinh", "display", "screen", "lcd", "led"], category: "computerOffice", vi: "Màn hình máy tính", en: "Computer monitor" },
  { keywords: ["keyboard", "ban phim", "mechanical keyboard"], category: "computerOffice", vi: "Bàn phím máy tính", en: "Computer keyboard" },
  { keywords: ["mouse", "chuot"], category: "computerOffice", vi: "Chuột máy tính", en: "Computer mouse" },
  { keywords: ["laptop", "macbook", "notebook"], category: "computerOffice", vi: "Laptop", en: "Laptop" },
  { keywords: ["iphone", "android", "smartphone", "dien thoai", "phone", "tablet", "ipad"], category: "phoneTablet", vi: "Điện thoại / máy tính bảng", en: "Phone / tablet" },
  { keywords: ["camera", "drone", "flycam", "gimbal"], category: "cameraDrone", vi: "Thiết bị quay chụp", en: "Camera / drone gear" },
  { keywords: ["pajama", "pyjama", "sleepwear", "quan ngu", "do ngu"], category: "fashion", vi: "Đồ ngủ", en: "Sleepwear" },
  { keywords: ["sneaker", "giay", "shoe", "sandal"], category: "footwear", vi: "Giày dép", en: "Footwear" },
  { keywords: ["bag", "tui", "backpack", "wallet", "vi"], category: "bags", vi: "Túi / ví", en: "Bag / wallet" },
  { keywords: ["detergent", "nuoc giat", "vien giat", "lau san", "household"], category: "householdEssentials", vi: "Sản phẩm tiêu dùng nhanh", en: "Household essentials" },
  { keywords: ["air fryer", "noi chien", "vacuum", "may hut bui", "blender", "home appliance"], category: "homeAppliances", vi: "Điện gia dụng", en: "Home appliance" },
  { keywords: ["tool", "khoan", "tua vit", "hardware", "dung cu"], category: "toolsHardware", vi: "Dụng cụ / hardware", en: "Tools / hardware" }
];

function estimateImageQuality(image = {}) {
  const src = String(image?.src || "").trim();
  const name = String(image?.name || "").toLowerCase();
  const hasDataUrl = /^data:image\/(png|jpeg|jpg|gif|webp);base64,/i.test(src);
  const dataLength = hasDataUrl ? src.length : 0;
  const looksLikeScreenshot = /(screenshot|screen-shot|screen_shot|zalo|messenger|facebook|chrome|safari|desktop|capture)/i.test(name);
  const looksTiny = dataLength > 0 && dataLength < MIN_STRONG_IMAGE_DATA_URL_LENGTH;

  let score = 0;
  if (hasDataUrl && dataLength >= MIN_SUGGEST_IMAGE_DATA_URL_LENGTH) score += 1;
  if (dataLength >= MIN_STRONG_IMAGE_DATA_URL_LENGTH) score += 1;
  if (!looksLikeScreenshot) score += 1;
...
function getImageQualitySummary(images = []) {
  const list = Array.isArray(images) ? images.slice(0, MAX_SUGGEST_IMAGE_PAYLOAD_COUNT) : [];
  const entries = list.map((image) => estimateImageQuality(image));
  const validCount = entries.filter((item) => item.hasDataUrl && item.dataLength >= MIN_SUGGEST_IMAGE_DATA_URL_LENGTH).length;
  const strongCount = entries.filter((item) => item.score >= 2).length;
  const lowQualityCount = entries.filter((item) => item.looksTiny || item.looksLikeScreenshot).length;
  return {
    total: entries.length,
    validCount,
    strongCount,
    lowQualityCount,
    hasEnoughSignal: validCount >= 1,
    hasStrongSignal: strongCount >= 1,
    entries
  };
}
...
const MOOD_ENFORCEMENT_GUIDES = {
  vi: [
    [
      "Giữ nhịp câu gọn, sáng, sạch; hạn chế cảm thán.",
      "Ưu tiên từ vựng trung tính - tinh tế, tránh đẩy cảm xúc quá cao.",
      "CTA nhẹ nhàng, thiên về 'tham khảo' và 'cân nhắc'."
    ],
    [
      "Giọng thân thiện như tư vấn 1-1, dùng ngôi 'bạn' tự nhiên.",
      "Chèn 1-2 tín hiệu đồng cảm đời thường (đi làm, chăm con, di chuyển...).",
      "CTA mang tính mời gọi mềm (xem thêm, lưu lại, tham khảo)."
    ],
    [
      "Nhịp câu nhanh hơn, động từ hành động rõ (vào việc, chốt nhanh, dùng ngay).",
      "Hook và đoạn 2 nên có năng lượng tích cực, cảm giác 'đẩy tiến độ'.",
      "CTA có độ thôi thúc cao hơn nhưng vẫn lịch sự."
    ],
    [
      "Luận điểm rõ theo cấu trúc lợi ích -> bằng chứng -> phù hợp ai.",
      "Dùng từ chắc nhịp, dứt khoát, hạn chế rào đón.",
      "CTA thiên về quyết định mua (thêm giỏ, chốt đơn, chọn ngay)."
    ]
  ],
  en: [
    [
      "Keep sentence rhythm clean and polished with minimal exclamation.",
      "Use refined neutral wording and avoid emotional overstatement.",
      "Use a soft consideration CTA."
    ],
    [
      "Write like a warm 1:1 advisor using natural second-person voice.",
      "Include 1-2 everyday empathy cues (workday, commute, family routine).",
      "Use gentle invitation CTA language."
    ],
    [
      "Increase pacing with stronger action verbs and momentum.",
      "Hook + paragraph 2 should feel lively and progress-oriented.",
      "CTA should be more urgent but still brand-safe."
    ],
    [
      "Structure claims as benefit -> evidence -> buyer fit with confidence.",
      "Use assertive wording and reduce hedging.",
      "CTA should lean toward conversion action (add to cart, choose now)."
    ]
  ]
};

const OPENING_WORD_BANLIST = {
  vi: [
    "sản phẩm này",
    "đây là",
    "nếu bạn đang tìm",
...
function inferCategoryFromProductName(productName = "") {
  return inferCategoryFromNameShared(productName) || null;
}

function normalizeSignalText(value = "") {
  return normalizeCompareText(String(value || "").replace(/[_-]+/g, " "));
}

function tokenizeSignalText(value = "") {
  return normalizeSignalText(value).split(/[^a-z0-9]+/).filter(Boolean);
}
...
const GENERIC_PRODUCT_NAME_MARKERS = [
  "san pham",
  "product",
  "item",
  "hang hoa",
...
const CATEGORY_OBJECT_LABELS = {
  vi: {
    computerOffice: "Thiết bị máy tính",
    electronics: "Thiết bị điện tử",
    phoneTablet: "Điện thoại",
    cameraDrone: "Thiết bị quay chụp",
    fashion: "Sản phẩm thời trang",
    skincare: "Sản phẩm chăm sóc da",
    beautyTools: "Dụng cụ làm đẹp",
    fragrance: "Nước hoa",
    homeAppliances: "Điện gia dụng",
    furnitureDecor: "Sản phẩm nội thất"
  },
  en: {
    computerOffice: "Computer device",
    electronics: "Electronics device",
    phoneTablet: "Phone",
    cameraDrone: "Camera gear",
    fashion: "Fashion product",
    skincare: "Skincare product",
    beautyTools: "Beauty tool",
    fragrance: "Perfume",
    homeAppliances: "Home appliance",
    furnitureDecor: "Furniture product"
  }
};

function getCategoryObjectLabel(category = "other", lang = "vi") {
  const langKey = getLangKey(lang) === "vi" ? "vi" : "en";
  return CATEGORY_OBJECT_LABELS[langKey]?.[String(category || "other")] || "";
}

function isGenericGeneratedProductName(value = "") {
  const normalized = normalizeCompareText(value);
  if (!normalized) return true;
  if (isUnknownGeneratedProductName(normalized)) return true;

  const tokenCount = normalized.split(/\s+/).filter(Boolean).length;
  if (tokenCount <= 1 && GENERIC_PRODUCT_NAME_MARKERS.includes(normalized)) {
    return true;
  }

  return GENERIC_PRODUCT_NAME_MARKERS.some((marker) => (
    normalized === marker || (normalized.startsWith(`${marker} `) && tokenCount <= 3)
  ));
}

function normalizeGeneratedNameHint(rawName = "", lang = "vi") {
  const raw = compact(rawName);
  if (!raw || isUnknownGeneratedProductName(raw)) return "";
  const normalized = normalizeGeneratedProductName(raw, lang);
  if (!normalized || isUnknownGeneratedProductName(normalized) || isGenericGeneratedProductName(normalized)) {
    return "";
  }
  return compact(normalized).slice(0, 120);
}

function composeGeneratedNameFromSearchFirst(searchFirst = {}, lang = "vi") {
  const source = searchFirst && typeof searchFirst === "object" ? searchFirst : {};
  const langKey = getLangKey(lang);
  const objectType = compact(source.objectType || getCategoryObjectLabel(source.category || "other", lang)).slice(0, 80);
  const brandName = compact(source.brandName || "").slice(0, 48);
  const modelName = compact(source.modelName || "").slice(0, 48);
  const sizeInfo = compact(source.sizeInfo || "").slice(0, 32);
  if (!objectType && !brandName && !modelName) return "";
  ...
  if (langKey === "vi") {
    pushUnique(objectType);
    pushUnique(brandName);
    pushUnique(modelName);
  } else {
    pushUnique(brandName);
    pushUnique(modelName);
    pushUnique(objectType);
  ...
  if (sizeInfo && /\d/.test(sizeInfo) && /(inch|in|cm|mm|hz|mah|w|ml|l|kg|g|tb|gb|x|×)/i.test(sizeInfo)) {
    pushUnique(sizeInfo);
  }

  const candidate = compact(picked.join(" ")).slice(0, 120);
  if (!candidate || isUnknownGeneratedProductName(candidate) || isGenericGeneratedProductName(candidate)) {
    return "";
  }

  if (brandName && objectType) {
    const normBrand = normalizeCompareText(brandName);
    const normObject = normalizeCompareText(objectType);
    if (normBrand && normObject) {
      const duplicatePattern = new RegExp(`(^|\\b)${escapeRegex(normBrand)}(\\b|$)`, "i");
      if (duplicatePattern.test(normObject)) {
        return compact(candidate.replace(new RegExp(`\\b${escapeRegex(brandName)}\\b`, "i"), "")).replace(/\s+/g, " ").trim();
      }
    }
  }
...
function inferBrandFromSignalText(signalText = "") {
  const normalized = normalizeSignalText(signalText);
  if (!normalized) return "";

  const tokens = tokenizeSignalText(normalized);
  const catalog = [...SEARCH_FIRST_BRAND_CATALOG].sort((left, right) => right.length - left.length);

  for (const brand of catalog) {
    const b = normalizeSignalText(brand);
    if (!b) continue;
    const exactPattern = new RegExp(`(^|\\b)${b.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(\\b|$)`, "i");
    if (exactPattern.test(normalized)) {
      return brand;
    }
    const tokenHit = tokens.find((token) => {
      if (token === b) return true;
      if (!token.startsWith(b)) return false;
      const suffix = token.slice(b.length);
      return /^\d{1,5}[a-z0-9]*$/i.test(suffix);
    });
    if (tokenHit) {
      return brand;
    }
...
function inferObjectHintFromSignalText(signalText = "", lang = "vi") {
  const normalized = normalizeSignalText(signalText);
  if (!normalized) {
    return { objectType: "", category: "other", evidence: [] };
  }

  const evidence = [];
  let best = { score: 0, category: "other", vi: "", en: "" };

  for (const item of SEARCH_FIRST_OBJECT_HINTS) {
    const hits = item.keywords.filter((keyword) => normalized.includes(normalizeSignalText(keyword)));
    const score = hits.length;
    if (score > 0) {
      evidence.push(...hits.slice(0, 2));
    }
    if (score > best.score) {
      best = { score, category: item.category, vi: item.vi, en: item.en };
    }
  }
  ...
  return {
    objectType: getLangKey(lang) === "vi" ? best.vi : best.en,
    category: best.category,
    evidence: Array.from(new Set(evidence)).slice(0, 3)
  };
}

function inferSizeFromSignalText(signalText = "") {
  const raw = String(signalText || "");
  if (!raw) return "";

  const dim = raw.match(/\b\d{1,4}(?:[.,]\d{1,2})?\s?[x×]\s?\d{1,4}(?:[.,]\d{1,2})?(?:\s?[x×]\s?\d{1,4}(?:[.,]\d{1,2})?)?\s?(?:mm|cm|m|inch|in|\")?\b/i);
  if (dim?.[0]) return compact(dim[0]);

  const unit = raw.match(/\b\d{1,4}(?:[.,]\d{1,2})?\s?(?:inch|in|\"|cm|mm|m|ml|l|kg|g|oz|tb|gb|mah|w|hz)\b/i);
  if (unit?.[0]) return compact(unit[0]);

  return "";
}

function inferColorFromSignalText(signalText = "", lang = "vi") {
  const normalized = normalizeSignalText(signalText);
  if (!normalized) return "";
  const isVi = getLangKey(lang) === "vi";
  for (const item of FACT_COLOR_DICTIONARY) {
...
export function buildSearchFirstClassifier(payload = {}, imageQuality = null) {
  const imageNames = (Array.isArray(payload?.images) ? payload.images : [])
    .map((image) => String(image?.name || ""))
    .filter(Boolean)
    .join(" ");
  const productName = String(payload?.productName || "");
  const mergedSignal = `${imageNames} ${productName}`.trim();
  const lang = payload?.lang || "vi";

  const signalSource = productName;

  const objectHint = inferObjectHintFromSignalText(signalSource, lang);
  const brandName = inferBrandFromSignalText(signalSource);
  const color = inferColorFromSignalText(signalSource, lang);
  const sizeInfo = inferSizeFromSignalText(signalSource);
  const modelName = inferModelFromText(signalSource);

  const inferredByName = inferCategoryFromProductName(productName) || "";
  let category = objectHint.category || "other";
  if (inferredByName && inferredByName !== "other") {
    if (category === "other") {
      category = inferredByName;
    } else if (shouldPreferInferredCategory(inferredByName, category)) {
      category = inferredByName;
    }
  }

  const evidence = [];
  if (brandName) evidence.push(`brand:${brandName}`);
  if (objectHint.objectType) evidence.push(`object:${objectHint.objectType}`);
  if (color) evidence.push(`color:${color}`);
  if (sizeInfo) evidence.push(`size:${sizeInfo}`);
  if (modelName) evidence.push(`model:${modelName}`);
  for (const item of objectHint.evidence || []) {
    evidence.push(`signal:${item}`);
  }

  const qualityBoost = imageQuality?.hasStrongSignal ? 0.1 : 0;
  const qualityPenalty = imageQuality && !imageQuality.hasEnoughSignal ? 0.2 : 0;
  const rawScore = Math.max(0, Math.min(1, 0.34 + (evidence.length * 0.08) + qualityBoost - qualityPenalty));
  const confidence = category === "other"
    ? Math.min(0.72, rawScore)
    : Math.max(0.58, Math.min(0.92, rawScore + 0.12));

  return {
    objectType: objectHint.objectType || "",
    category: category || "other",
    generatedProductNameHint: "",
    brandName,
    modelName,
    color,
    sizeInfo,
...
function mergeSearchFirstFacts(baseFacts = {}, searchFirst = {}) {
  const facts = buildEmptySuggestionFacts(baseFacts);
  const searchSpecs = asStringArray(searchFirst?.keySpecs, { max: 6, maxLength: 120 });
  const searchFeatures = asStringArray(searchFirst?.specialFeatures, { max: 6, maxLength: 120 });
  const merged = {
    ...facts,
    brandName: compact(facts.brandName || searchFirst.brandName || ""),
    modelName: compact(facts.modelName || searchFirst.modelName || ""),
    color: compact(facts.color || searchFirst.color || ""),
    sizeInfo: compact(facts.sizeInfo || searchFirst.sizeInfo || ""),
    material: compact(facts.material || searchFirst.material || ""),
    packSize: compact(facts.packSize || searchFirst.packSize || ""),
    keySpecs: asStringArray([...(facts.keySpecs || []), ...searchSpecs], { max: 6, maxLength: 120 }),
    specialFeatures: asStringArray([...(facts.specialFeatures || []), ...searchFeatures], { max: 6, maxLength: 120 }),
    evidenceLevel: normalizeFactEvidenceLevel(facts.evidenceLevel, Number(searchFirst?.confidence || 0))
  };
  return buildEmptySuggestionFacts(merged);
}

function getSearchFirstNote(searchFirst = {}, lang = "vi") {
  const signalBits = Array.isArray(searchFirst?.evidence) ? searchFirst.evidence : [];
  if (!signalBits.length) {
    return "";
  }
  if (getLangKey(lang) === "vi") {
    return `Visual-first: đã nhận diện sơ bộ ${Math.min(signalBits.length, 4)} tín hiệu từ ảnh (${signalBits.slice(0, 4).join(", ")}).`;
  }
  return `Visual-first: detected ${Math.min(signalBits.length, 4)} preliminary image signals (${signalBits.slice(0, 4).join(", ")}).`;
}

function normalizeVisualEvidence(raw = {}, lang = "vi") {
  const source = raw && typeof raw === "object" ? raw : {};
  const categoryHint = normalizeCategoryValue(source.categoryHint || source.category || "other");
  const objectType = compact(source.objectType || source.productType || source.productClass || "").slice(0, 100);
  const brandName = compact(source.brandName || source.brand || "").slice(0, 90);
  const modelName = compact(source.modelName || source.model || "").slice(0, 90);
  const color = compact(source.color || source.colorHint || "").slice(0, 90);
  const sizeInfo = compact(source.sizeInfo || source.size || source.dimension || "").slice(0, 90);
  const material = compact(source.material || "").slice(0, 90);
  const packSize = compact(source.packSize || source.pack || "").slice(0, 90);
  const keySpecs = asStringArray(source.keySpecs || source.specs, { max: 6, maxLength: 120 });
  const specialFeatures = asStringArray(source.specialFeatures || source.features, { max: 6, maxLength: 120 });
  const visibleTexts = asStringArray(source.visibleTexts || source.visibleText || source.ocrTexts || source.ocrText, { max: 8, maxLength: 80 });
  const notes = asStringArray(source.notes, { max: 4, maxLength: 120 });
  const confidence = Math.max(0, Math.min(1, Number(source.confidence || source.visualConfidence || 0)));
  const generatedProductNameHint = normalizeGeneratedProductName(source.generatedProductNameHint || source.generatedProductName || "", lang);
...
function mergeSearchFirstWithVisualEvidence(searchFirst = {}, visualEvidence = {}) {
  const base = searchFirst && typeof searchFirst === "object" ? searchFirst : {};
  const visual = visualEvidence && typeof visualEvidence === "object" ? visualEvidence : {};

  const category = visual.categoryHint && visual.categoryHint !== "other"
    ? visual.categoryHint
    : (base.category || "other");
  ...
  if (visual.objectType) evidence.push(`vision-object:${visual.objectType}`);
  if (visual.brandName) evidence.push(`vision-brand:${visual.brandName}`);
  if (visual.modelName) evidence.push(`vision-model:${visual.modelName}`);
  if (visual.color) evidence.push(`vision-color:${visual.color}`);
  if (visual.sizeInfo) evidence.push(`vision-size:${visual.sizeInfo}`);
  for (const text of Array.isArray(visual.visibleTexts) ? visual.visibleTexts.slice(0, 3) : []) {
    evidence.push(`ocr:${text}`);
  }
  ...
  const confidence = Math.max(
    Number(base.confidence || 0),
    Number(visual.confidence || 0),
    category !== "other" && Number(visual.confidence || 0) > 0.42 ? 0.62 : 0
  );

  return {
    objectType: visual.objectType || base.objectType || "",
    category,
    generatedProductNameHint: visual.generatedProductNameHint || base.generatedProductNameHint || "",
    brandName: visual.brandName || base.brandName || "",
    modelName: visual.modelName || base.modelName || "",
    color: visual.color || base.color || "",
    sizeInfo: visual.sizeInfo || base.sizeInfo || "",
    material: visual.material || base.material || "",
    packSize: visual.packSize || base.packSize || "",
    keySpecs: asStringArray([...(Array.isArray(base.keySpecs) ? base.keySpecs : []), ...(Array.isArray(visual.keySpecs) ? visual.keySpecs : [])], { max: 6, maxLength: 120 }),
    specialFeatures: asStringArray([...(Array.isArray(base.specialFeatures) ? base.specialFeatures : []), ...(Array.isArray(visual.specialFeatures) ? visual.specialFeatures : [])], { max: 6, maxLength: 120 }),
    notes: asStringArray([...(Array.isArray(base.notes) ? base.notes : []), ...(Array.isArray(visual.notes) ? visual.notes : [])], { max: 4, maxLength: 120 }),
    evidence: uniqueEvidence.slice(0, 8),
    confidence: Math.max(0, Math.min(1, confidence))
  };
}

const FACT_COLOR_DICTIONARY = [
  { key: "black", vi: "Đen", en: "Black", pattern: /\b(black|den)\b/i },
  { key: "white", vi: "Trắng", en: "White", pattern: /\b(white|trang|tr\w*ng)\b/i },
  { key: "gray", vi: "Xám", en: "Gray", pattern: /\b(gray|grey|xam)\b/i },
  { key: "silver", vi: "Bạc", en: "Silver", pattern: /\b(silver|bac)\b/i },
  { key: "gold", vi: "Vàng", en: "Gold", pattern: /\b(gold|vang)\b/i },
  { key: "blue", vi: "Xanh dương", en: "Blue", pattern: /\b(blue|xanh\s*duong|xanhduong)\b/i },
  { key: "green", vi: "Xanh lá", en: "Green", pattern: /\b(green|xanh\s*la|xanhla)\b/i },
  { key: "red", vi: "Đỏ", en: "Red", pattern: /\b(red|do)\b/i },
  { key: "pink", vi: "Hồng", en: "Pink", pattern: /\b(pink|hong)\b/i },
  { key: "purple", vi: "Tím", en: "Purple", pattern: /\b(purple|tim)\b/i },
  { key: "brown", vi: "Nâu", en: "Brown", pattern: /\b(brown|nau)\b/i },
  { key: "beige", vi: "Be", en: "Beige", pattern: /\b(beige|be)\b/i }
];

const BRAND_TOKEN_EXCLUDE = new Set([
  "may", "man", "hinh", "monitor", "screen", "display", "product", "san", "pham",
  "tai", "nghe", "wireless", "headphone", "smart", "phone", "case", "cover", "set"
]);

function inferColorFromText(text = "", lang = "vi") {
...
function inferSizeFromText(text = "") {
  ...
  const dimensionMatch = source.match(/\b\d{1,4}\s?[x×]\s?\d{1,4}(?:\s?[x×]\s?\d{1,4})?\s?(?:mm|cm|m|inch|in|\")?\b/i);
  if (dimensionMatch?.[0]) return compact(dimensionMatch[0]);

  const unitMatch = source.match(/\b\d{1,4}(?:[.,]\d{1,2})?\s?(?:inch|in|\"|cm|mm|m|ml|l|kg|g|oz|tb|gb|mah|w|hz)\b/i);
  if (unitMatch?.[0]) return compact(unitMatch[0]);

  return "";
}

function inferModelFromText(text = "") {
  const source = String(text || "");
  if (!source) return "";

  const match = source.match(/\b([A-Za-z]{1,6}[\-_]?[A-Za-z0-9]{2,8})\b/g);
  if (!Array.isArray(match) || !match.length) return "";

  const candidate = match.find((item) => /\d/.test(item) && /[A-Za-z]/.test(item)) || "";
  return compact(candidate).slice(0, 80);
}
...
function inferBrandFromText(text = "") {
  ...
  const knownBrands = [
    "Apple", "Samsung", "Xiaomi", "Oppo", "Vivo", "Sony", "LG", "Dell", "HP", "Asus", "Acer", "Lenovo",
    "MSI", "Logitech", "Razer", "JBL", "Anker", "Baseus", "Corsair", "ViewSonic", "AOC", "BenQ", "Philips", "Panasonic"
  ];
  const lowered = source.toLowerCase();
  const known = knownBrands.find((brand) => lowered.includes(brand.toLowerCase()));
  if (known) return known;

  const firstToken = source.split(/\s+/)[0] || "";
  const tokenNormalized = normalizeCompareText(firstToken);
  if (!firstToken || BRAND_TOKEN_EXCLUDE.has(tokenNormalized)) return "";
  if (/^[A-Za-z][A-Za-z0-9-]{1,18}$/.test(firstToken)) {
    return firstToken;
  }
  return "";
}

function buildCategoryAdvancedFacts(category = "other", lang = "vi", facts = {}) {
  const isVi = getLangKey(lang) === "vi";
  const safeSize = compact(facts?.sizeInfo || "");
  const safeMaterial = compact(facts?.material || "");
  const firstSpec = Array.isArray(facts?.keySpecs) && facts.keySpecs.length ? String(facts.keySpecs[0] || "") : "";
  const firstFeature = Array.isArray(facts?.specialFeatures) && facts.specialFeatures.length ? String(facts.specialFeatures[0] || "") : "";

  const next = sanitizeSuggestAdvancedFields({});

  if (["electronics", "computerOffice", "phoneTablet", "cameraDrone", "autoMoto", "digitalGoods"].includes(category)) {
    next.specs = compact([safeSize, firstSpec, firstFeature].filter(Boolean).join(" · "));
    next.compatibility = compact(facts?.targetUseContext || "");
    next.warranty = compact(facts?.condition || "");
    return next;
  }

  if (["home", "furnitureDecor", "homeAppliances", "toolsHardware", "householdEssentials"].includes(category)) {
    next.dimensions = safeSize;
    next.usageSpace = compact(facts?.targetUseContext || "");
    next.warranty = compact(facts?.condition || "");
    return next;
  }

  if (["skincare", "beautyTools", "healthCare", "motherBaby"].includes(category)) {
    next.usage = compact(facts?.targetUseContext || "");
    next.skinConcern = compact(firstFeature || "");
    next.routineStep = compact(firstSpec || "");
    return next;
  }

  if (["fashion", "footwear", "bags", "accessories", "fragrance"].includes(category)) {
    next.sizeGuide = safeSize;
    next.careGuide = safeMaterial
      ? (isVi ? `Chất liệu tham chiếu: ${safeMaterial}` : `Material reference: ${safeMaterial}`)
      : "";
    next.exchangePolicy = compact(facts?.condition || "");
    return next;
  }
...
function buildFactsFromNameAndCategory({ productName = "", category = "other", lang = "vi" } = {}) {
  const normalizedName = compact(productName || "");
  const generatedCategory = category || inferCategoryFromProductName(normalizedName) || "other";

  const brandName = inferBrandFromText(normalizedName);
  const modelName = inferModelFromText(normalizedName);
  const color = inferColorFromText(normalizedName, lang);
  const sizeInfo = inferSizeFromText(normalizedName);

  const keySpecs = [];
  if (sizeInfo) {
    keySpecs.push(getLangKey(lang) === "vi" ? `Kích thước tham chiếu: ${sizeInfo}` : `Reference size: ${sizeInfo}`);
  }

  const facts = buildEmptySuggestionFacts({
    brandName,
    modelName,
    color,
    sizeInfo,
    ...
    keySpecs,
    specialFeatures: [],
    evidenceLevel: normalizedName ? "medium" : "low"
  });

  facts.advanced = buildCategoryAdvancedFacts(generatedCategory, lang, facts);
  return facts;
}

function mergeSuggestionFacts(primary = {}, fallback = {}) {
  const base = buildEmptySuggestionFacts(primary);
  const backup = buildEmptySuggestionFacts(fallback);
  const merged = buildEmptySuggestionFacts();

  const scalarKeys = ["brandName", "modelName", "color", "sizeInfo", "material", "packSize", "condition", "targetUseContext"];
  for (const key of scalarKeys) {
    merged[key] = compact(base[key] || backup[key] || "");
  }

  merged.keySpecs = asStringArray((base.keySpecs || []).concat(backup.keySpecs || []), { max: 6, maxLength: 120 });
  merged.specialFeatures = asStringArray((base.specialFeatures || []).concat(backup.specialFeatures || []), { max: 6, maxLength: 120 });
  merged.advanced = sanitizeSuggestAdvancedFields(base.advanced, backup.advanced);
  merged.evidenceLevel = normalizeFactEvidenceLevel(base.evidenceLevel || backup.evidenceLevel, 0);
  return merged;
}

function enrichSuggestionWithFacts(suggestion = {}, lang = "vi") {
  if (!suggestion || typeof suggestion !== "object") return suggestion;
  const isVi = getLangKey(lang) === "vi";
  const facts = buildEmptySuggestionFacts(suggestion.facts || {});

  const highlightCandidates = asStringArray([
    ...facts.specialFeatures,
    ...facts.keySpecs,
    facts.targetUseContext
  ], { max: 8, maxLength: 120 });

  const attributeCandidates = [
    facts.brandName ? (isVi ? `Thương hiệu: ${facts.brandName}` : `Brand: ${facts.brandName}`) : "",
    facts.modelName ? (isVi ? `Model: ${facts.modelName}` : `Model: ${facts.modelName}`) : "",
    facts.color ? (isVi ? `Màu sắc: ${facts.color}` : `Color: ${facts.color}`) : "",
    facts.sizeInfo ? (isVi ? `Kích thước: ${facts.sizeInfo}` : `Size: ${facts.sizeInfo}`) : "",
    facts.material ? (isVi ? `Chất liệu: ${facts.material}` : `Material: ${facts.material}`) : "",
    facts.packSize ? (isVi ? `Quy cách: ${facts.packSize}` : `Pack size: ${facts.packSize}`) : "",
    facts.condition ? (isVi ? `Tình trạng: ${facts.condition}` : `Condition: ${facts.condition}`) : ""
  ].filter(Boolean);

  const currentHighlights = asStringArray(suggestion.highlights, { max: 6, maxLength: 120 });
  const mergedHighlights = asStringArray([...currentHighlights, ...highlightCandidates], { max: 6, maxLength: 120 });
  suggestion.highlights = mergedHighlights;

  const currentAttributes = Array.isArray(suggestion.attributes)
    ? suggestion.attributes
        .map((item, index) => ({ type: toNumberOr(item?.type, index), value: compact(item?.value || "") }))
        .filter((item) => item.value)
    : [];
  const existingAttributeValues = new Set(currentAttributes.map((item) => normalizeCompareText(item.value)));
  const extraAttributes = [];
  for (const line of attributeCandidates) {
    const key = normalizeCompareText(line);
    if (!key || existingAttributeValues.has(key)) continue;
    existingAttributeValues.add(key);
    extraAttributes.push({ type: currentAttributes.length + extraAttributes.length, value: line });
    if (currentAttributes.length + extraAttributes.length >= 6) break;
  }
  suggestion.attributes = [...currentAttributes, ...extraAttributes].slice(0, 6);

  if (!compact(suggestion.shortDescription) && compact(facts.targetUseContext)) {
    suggestion.shortDescription = compact(facts.targetUseContext);
  }
...
function buildNameOnlySuggestion(payload, outputConfig) {
  ...
  const baseFacts = buildFactsFromNameAndCategory({
    productName: payload?.productName || "",
    category: suggestion.category,
    lang: payload?.lang || "vi"
  });
  suggestion.facts = baseFacts;
  return enrichSuggestionWithFacts(suggestion, payload?.lang || "vi");
}

function buildImageOnlyHeuristicSuggestion(payload, outputConfig) {
  const lang = getLangKey(payload?.lang);
  const isVi = lang === "vi";
  const category = inferCategoryFromProductName(payload?.productName || "") || "other";
  ...
  const suggestion = {
    ...
    shortDescription: isVi
      ? "Suy luận tạm từ phân tích ảnh chưa ổn định; cần thêm ảnh rõ hơn để nhận diện chính xác."
      : "Temporary inference from unstable image analysis; provide clearer photos for higher precision.",
    highlights: [],
    attributes: [],
    confidence: category === "other" ? 0.32 : 0.52,
    notes: [isVi
      ? "Đã suy luận best-effort từ tín hiệu ảnh hiện có do phân tích trực tiếp chưa ổn định."
      : "Best-effort inference from current image signals because direct vision analysis was unstable."]
  };

  const baseFacts = buildFactsFromNameAndCategory({
    productName: payload?.productName || generatedProductName,
    category: suggestion.category,
    lang: payload?.lang || "vi"
  });
  suggestion.facts = baseFacts;
...
function shouldOverrideSuggestedCategoryByName(productName = "", suggestedCategory = "") {
  ...
  return shouldPreferInferredCategory(inferredCategory, suggestedCategory);
}

function normalizeCategoryValue(rawCategory = "") {
  const normalizedByShared = normalizeSuggestedCategory(rawCategory);
  if (normalizedByShared !== "other") {
    return normalizedByShared;
  }
...
function normalizeGeneratedProductName(rawName = "", lang = "vi") {
  ...
  if (isUnknownGeneratedProductName(compacted)) {
    return getLangKey(lang) === "vi" ? "Không nhận dạng tên sản phẩm được" : "Unable to identify product name";
  }
...
function hasNoDataSuggestionNote(notes, outputConfig) {
  const entries = Array.isArray(notes) ? notes : [];
  if (!entries.length) return false;

  const fallbackNote = normalizeCompareText(outputConfig?.suggestNoDataNote || "");
  const markers = [
    fallbackNote,
    "khong du du lieu anh",
    "anh tai len chua du",
  ...
  return entries.some((note) => {
    const normalized = normalizeCompareText(note || "");
    if (!normalized) return false;
    return markers.some((marker) => normalized.includes(marker));
  });
}

function repairWeakSuggestionWithNameContext(suggestion, payload, outputConfig, imageQuality) {
  let repaired = { ...(suggestion || {}) };
  const nameBased = buildNameOnlySuggestion(payload, outputConfig);
  const isVi = getLangKey(payload?.lang) === "vi";
  const hasStrongSignal = Boolean(imageQuality?.hasStrongSignal);

  repaired.category = normalizeCategoryValue(repaired.category || nameBased.category || "other");
  if (repaired.category === "other" && nameBased.category !== "other") {
    repaired.category = nameBased.category;
  }

  repaired.tone = toNumberOr(repaired.tone, nameBased.tone);
  repaired.channel = toNumberOr(repaired.channel, nameBased.channel);
  repaired.mood = toNumberOr(repaired.mood, nameBased.mood);
  repaired.brandStyle = toNumberOr(repaired.brandStyle, nameBased.brandStyle);

  const generatedName = normalizeGeneratedProductName(repaired.generatedProductName || "", payload?.lang);
  repaired.generatedProductName = isUnknownGeneratedProductName(generatedName)
    ? nameBased.generatedProductName
    : generatedName;

  if (!compact(repaired.targetCustomer)) repaired.targetCustomer = nameBased.targetCustomer;
  if (!compact(repaired.shortDescription)) repaired.shortDescription = nameBased.shortDescription;
  if (!Array.isArray(repaired.highlights) || !repaired.highlights.length) repaired.highlights = nameBased.highlights.slice(0, 3);
  if (!Array.isArray(repaired.attributes) || !repaired.attributes.length) repaired.attributes = nameBased.attributes.slice(0, 3);

  const existingNotes = Array.isArray(repaired.notes)
    ? repaired.notes.filter((note) => !hasNoDataSuggestionNote([note], outputConfig))
    : [];
  const recoveryNote = hasStrongSignal
    ? (isVi
      ? "AI đã kết hợp tín hiệu ảnh rõ và tên sản phẩm để tăng độ chính xác gợi ý."
      : "AI combined clear image cues with product name to improve suggestion accuracy.")
    : (isVi
      ? "AI đã kết hợp dữ liệu ảnh hiện có với tên sản phẩm để ổn định kết quả gợi ý."
      : "AI combined available image data with product name to stabilize the suggestion.");

  repaired.notes = [recoveryNote, ...existingNotes].slice(0, 4);

  const nameFacts = buildFactsFromNameAndCategory({
    productName: payload?.productName || repaired.generatedProductName || "",
    category: repaired.category,
    lang: payload?.lang || "vi"
  });
  repaired.facts = mergeSuggestionFacts(parseSuggestionFacts(repaired, repaired?.facts || {}), nameFacts);
  repaired = enrichSuggestionWithFacts(repaired, payload?.lang || "vi");

  const currentConfidence = Math.max(0, Math.min(1, Number(repaired.confidence) || 0));
  const minimumConfidence = hasStrongSignal ? 0.62 : 0.52;
  repaired.confidence = Math.max(currentConfidence, minimumConfidence);
  if (repaired.category === "other") {
    repaired.confidence = Math.min(repaired.confidence, 0.49);
  }
...
export function getVariantStyleLabel(styleKey = "", lang = "vi") {
  const normalized = String(styleKey || "").trim().toLowerCase();
  const isVi = getLangKey(lang) === "vi";

  if (normalized === "balanced") return isVi ? "Cân bằng" : "Balanced";
  if (normalized === "expert") return isVi ? "Chuyên gia" : "Expert";
  if (normalized === "sales") return isVi ? "Chốt sale" : "Sales";
  if (normalized === "lifestyle") return isVi ? "Lifestyle" : "Lifestyle";
...
function extractResponsesOutputText(response = null) {
  ...
  return [...parts, fallbackText].filter(Boolean).join("\n").trim();
}
...
async function postResponsesJson(url, body, headers = {}) {
...
function sanitizeSuggestAdvancedFields(value = {}, fallback = {}) {
  const source = value && typeof value === "object" ? value : {};
  const backup = fallback && typeof fallback === "object" ? fallback : {};
  const result = {};

  for (const key of SUGGEST_ADVANCED_FIELD_KEYS) {
    result[key] = compact(source?.[key] || backup?.[key] || "").slice(0, 180);
  }

  return result;
}

function normalizeFactEvidenceLevel(value = "", confidence = 0) {
...
function buildEmptySuggestionFacts(overrides = {}) {
  const base = {
    brandName: "",
    modelName: "",
    color: "",
    ...
    keySpecs: [],
    specialFeatures: [],
    advanced: sanitizeSuggestAdvancedFields({}),
    evidenceLevel: "low"
  };
  ...
  next.keySpecs = asStringArray(next.keySpecs, { max: 6, maxLength: 120 });
  next.specialFeatures = asStringArray(next.specialFeatures, { max: 6, maxLength: 120 });
  next.advanced = sanitizeSuggestAdvancedFields(next.advanced, base.advanced);
  next.evidenceLevel = normalizeFactEvidenceLevel(next.evidenceLevel, 0);
  return next;
}

function parseSuggestionFacts(raw = {}, fallback = null) {
  const source = raw && typeof raw === "object" ? raw : {};
  const factsRaw = source?.facts && typeof source.facts === "object" ? source.facts : {};
  const backup = fallback && typeof fallback === "object" ? fallback : {};

  const brandName = compact(factsRaw.brandName || source.brandName || backup.brandName || "").slice(0, 90);
  const modelName = compact(factsRaw.modelName || source.modelName || backup.modelName || "").slice(0, 90);
  const color = compact(factsRaw.color || source.color || backup.color || "").slice(0, 90);
  const sizeInfo = compact(factsRaw.sizeInfo || source.sizeInfo || backup.sizeInfo || "").slice(0, 90);
  const material = compact(factsRaw.material || source.material || backup.material || "").slice(0, 90);
  const packSize = compact(factsRaw.packSize || source.packSize || backup.packSize || "").slice(0, 90);
  const condition = compact(factsRaw.condition || source.condition || backup.condition || "").slice(0, 90);
  const targetUseContext = compact(factsRaw.targetUseContext || source.targetUseContext || backup.targetUseContext || "").slice(0, 140);
  const keySpecs = asStringArray(factsRaw.keySpecs ?? source.keySpecs ?? backup.keySpecs, { max: 6, maxLength: 120 });
  const specialFeatures = asStringArray(factsRaw.specialFeatures ?? source.specialFeatures ?? backup.specialFeatures, { max: 6, maxLength: 120 });
  const advanced = sanitizeSuggestAdvancedFields(factsRaw.advanced || source.advanced, backup.advanced || {});
  const confidenceSeed = Number(source.confidence || backup.confidence || 0);
  const evidenceLevel = normalizeFactEvidenceLevel(factsRaw.evidenceLevel || source.evidenceLevel || backup.evidenceLevel, confidenceSeed);

  return buildEmptySuggestionFacts({
    brandName,
    modelName,
    color,
    sizeInfo,
...
function hasMeaningfulSuggestionFacts(facts = {}) {
  const source = facts && typeof facts === "object" ? facts : {};
  const hasPrimary = Boolean(
    compact(source.brandName)
    || compact(source.modelName)
    || compact(source.color)
    || compact(source.sizeInfo)
    || compact(source.material)
    || compact(source.packSize)
    || compact(source.condition)
    || compact(source.targetUseContext)
  );
  const hasArrays = Boolean(
    (Array.isArray(source.keySpecs) && source.keySpecs.length)
    || (Array.isArray(source.specialFeatures) && source.specialFeatures.length)
  );
  const advanced = source.advanced && typeof source.advanced === "object" ? source.advanced : {};
  const hasAdvanced = SUGGEST_ADVANCED_FIELD_KEYS.some((key) => compact(advanced[key] || ""));
  return hasPrimary || hasArrays || hasAdvanced;
}
...
function hasPriceMention(text = "") {
  ...
  return /(gia|ph[aâ]n\s*kh[uú]c|price|budget|premium|mid-range|t[aầ]m\s*trung|gi[aá]\s*r[eẻ]|cao\s*c[aấ]p|\d+[\s,\.]*\d*\s*(k|tr|tri[eệ]u|vnd|đ|dong|usd|\$|eur))/i.test(source);
}

function normalizeVnNumericPriceText(priceText = "") {
  const raw = compact(priceText, 80);
  if (!raw) return "";
  const normalized = raw.toLowerCase().replace(/\s+/g, " ").trim();
  if (/(\d+\s*(k|tr|tri[eệ]u|vnd|đ|dong|usd|\$|eur))/i.test(normalized)) {
    return raw;
  }

  const nums = normalized.match(/\d{2,7}/g) || [];
  if (!nums.length) return raw;

  const first = Number(nums[0]);
  if (!Number.isFinite(first) || first <= 0) return raw;

  if (first >= 1000000) {
    const million = Math.round((first / 1000000) * 10) / 10;
    return `${million} triệu`;
  }
  if (first >= 1000) {
    const k = Math.round(first / 1000);
    return `${k}k`;
  }
  return `${first}k`;
}

function buildVnPriceAnalogy(priceLabel = "") {
  const text = String(priceLabel || "").toLowerCase();
  if (!text) return "";
  if (/45k|50k|4\s*[5-9]k|5\d\s*k/.test(text)) return "cỡ 1 ly trà sữa";
  if (/1\d\d\s*k|2\d\d\s*k|3\d\d\s*k|duoi\s*200k|chua\s*den\s*200k/.test(text)) return "cỡ 2-3 ly trà sữa";
  if (/500k|600k|700k|800k/.test(text)) return "cỡ một buổi đi cafe chill cuối tuần";
  if (/tri[eệ]u|\d\s*tr/.test(text)) return "cỡ một lần sắm đồ xịn cho bản thân";
  return "cỡ vài ly trà sữa";
}

function normalizePriceSegmentLabel(priceSegment = "", langKey = "vi") {
  ...
  if (key === "low") return "tầm giá dễ chốt";
  if (key === "mid") return "tầm trung";
  if (key === "high") return "phân khúc cao";
  return raw;
}

function buildPriceLine(payload = {}, langKey = "vi") {
  const labelRaw = normalizePriceSegmentLabel(payload?.priceSegment || "", langKey);
  const label = langKey === "vi" ? normalizeVnNumericPriceText(labelRaw) : labelRaw;
  if (!label) return "";
  if (langKey === "en") {
    return `• Price note: around ${label}, making this easier to justify for daily use.`;
  }
  const analogy = buildVnPriceAnalogy(label);
  if (analogy) {
    return `• Giá tham khảo: ${label} — nghe vui vui thì ${analogy}, nên chốt cũng đỡ lăn tăn.`;
  }
  return `• Giá tham khảo: ${label}, đủ dễ cân nhắc để chốt khi đúng nhu cầu.`;
}
...
function resultNeedsRetry(result) {
  ...
  if (compacted.some((item) => item.length < 20)) return true;

  const firstParagraph = compacted[0] || "";
  const firstSentence = firstParagraph.split(/[.!?\n]+/g).map((part) => compact(part)).filter(Boolean)[0] || "";
  if (!firstSentence) return true;

  const langKey = /[À-ỹà-ỹ]/.test(firstParagraph) ? "vi" : "en";
  const bannedList = OPENING_WORD_BANLIST[langKey] || [];
  const lowered = firstSentence.toLowerCase();
  if (bannedList.some((item) => lowered.includes(String(item).toLowerCase()))) {
    return true;
  }

  const wordCount = firstSentence.split(/\s+/g).filter(Boolean).length;
...
function evaluateOutputQuality(result, payload, langKey) {
  ...
  const firstSentence = p1.split(/[.!?\n]+/g).map((part) => compact(part)).filter(Boolean)[0] || "";
  const firstSentenceWords = firstSentence.split(/\s+/g).filter(Boolean).length;
  const openingBanlist = OPENING_WORD_BANLIST[langKey] || [];
  const hasBoringOpening = openingBanlist.some((item) => firstSentence.toLowerCase().includes(String(item).toLowerCase()));
  ...
  if (firstSentenceWords >= 8 && firstSentenceWords <= 24) scores.creativity += 45;
  if (!hasBoringOpening) scores.creativity += 60;
  if (/(bạn|you|vừa|ngay|chiếc|mẫu|lên đồ|chốt|setup|routine|deadline|ca sáng|ca tối|da dễ|desk|workday)/i.test(firstSentence)) {
    scores.creativity += 35;
  }
  ...
  if (/(comment|inbox|xem thêm|lưu lại|tham khảo|chốt|thêm giỏ|add to cart|save this)/i.test(p3)) {
    scores.conversion += 30;
  }
  ...
  if (hasBoringOpening) reasons.push(langKey === "en" ? "Opening line feels generic" : "Câu mở đầu còn chung chung");
  if (firstSentenceWords < 8 || firstSentenceWords > 28) {
    reasons.push(langKey === "en" ? "Opening line pacing is off" : "Nhịp câu mở đầu chưa tốt");
  }
...
function normalizeAiPayloadResult(payload, parsed) {
  ...
  if (payload.priceSegment) {
    const combined = thirdLines.join("\n");
    if (!hasPriceMention(combined)) {
      const priceLine = buildPriceLine(payload, langKey);
      if (priceLine) thirdLines.push(priceLine);
    }
  }
...
function buildPrompt(payload, options = {}) {
  ...
  const moodEnforcement = safePick(MOOD_ENFORCEMENT_GUIDES[langKey], payload.mood, []).join("\n- ");
  ...
  const openingHookRules = langKey === "en"
    ? [
        "- The very first sentence must hook within 1 line: concrete situation, tension, or surprising payoff.",
        "- First sentence must be 8-24 words, energetic, and not start with 'This product' / 'This is'.",
        "- Avoid generic openers and template phrases. Make the opening feel fresh and social-native.",
        "- Use youthful but brand-safe rhythm: short punch + specific context + practical benefit.",
        "- If opening sounds dull, rewrite it completely before finishing the output."
      ]
    : [
        "- Câu đầu tiên phải có lực hút trong 1 dòng: nêu tình huống cụ thể, điểm căng nhẹ, hoặc lợi ích bất ngờ.",
        "- Câu mở đầu dài 8-24 từ, nhịp nhanh, tránh mở bằng 'Sản phẩm này', 'Đây là'.",
        "- Tránh văn khuôn mẫu/công thức. Mở đầu phải mới, tự nhiên, đúng ngôn ngữ người dùng trẻ.",
        "- Giữ nhịp trẻ trung nhưng an toàn thương hiệu: câu gọn + bối cảnh thật + lợi ích rõ.",
        "- Nếu mở đầu còn nhạt, bắt buộc viết lại toàn bộ câu mở đầu trước khi trả kết quả."
      ];

  const promptQualityBlock = [...narrativeConstraints, ...antiGenericRules, ...openingHookRules].join("\n");
...
export async function generateProductCopy(payload) {
  ...
  const preferredModel = String(process.env.AI_MODEL || "cx/gpt-5.1").trim() || "cx/gpt-5.1";
  ...
  const forceAiOnly = String(process.env.AI_GENERATE_FORCE_AI_ONLY || "true").trim().toLowerCase() !== "false";
  ...
  try {
    ...
    const responsesEndpoint = `${normalizedBase}/responses`;
    let quality = null;
    ...
    const toFormattedResultFromParsedObject = (parsedObject) => {
      if (!parsedObject || typeof parsedObject !== "object") return null;
      const normalized = normalizeAiPayloadResult(payload, parsedObject);
      return applyCategoryFormatting(normalized, payload, langKey);
    };

    const requestWithModelFallback = async (promptText) => {
      let lastError = null;
      for (const model of modelCandidates) {
        const attemptContent = [{ type: "text", text: promptText }, ...content.slice(1)];
        const responsesBody = {
          model,
          store: false,
          stream: true,
          instructions: "You are a senior ecommerce copywriter. Return valid JSON only.",
          input: [{
            role: "user",
            content: attemptContent.map((part) => {
        ...
        try {
          const parsed = await postResponsesJson(responsesEndpoint, responsesBody, requestHeaders);
          const formattedFromResponses = toFormattedResultFromParsedObject(parsed);
          if (formattedFromResponses) {
            return formattedFromResponses;
          }
        } catch (error) {
          lastError = error;
        }
        ...
        try {
          const chatRaw = await postJson(chatEndpoint, chatBody, requestHeaders);
          const formattedFromChat = toFormattedResult(chatRaw);
          if (formattedFromChat) {
            return formattedFromChat;
          }
          lastError = new Error("chat_empty_output");
        } catch (error) {
          lastError = error;
        }
      }
    ...
    const runAttempt = async (attempt, previousQuality = null) => {
      ...
      const repairBlock = attempt > 1
        ...
        : "";
      const attemptPrompt = `${buildPrompt(payload, { previousQuality })}${repairBlock}`;
      const attemptResult = await requestWithModelFallback(attemptPrompt);
    ...
    if (!finalResult?.paragraphs?.length) {
      if (forceAiOnly) {
...
export async function generateProductCopyVariants(payload) {
  ...
  const variantCount = Math.max(1, Math.min(5, requested));
  const requestedStylePresets = Array.isArray(payload?.variantStylePresets)
  ...
  const startIndex = styleSequence.indexOf(String(payload.stylePreset || "balanced"));
  const normalizedStart = startIndex >= 0 ? startIndex : 0;
  const defaultStyleOrder = Array.from({ length: variantCount }).map((_, index) => styleSequence[(normalizedStart + index) % styleSequence.length] || "balanced");
  const finalVariantStyles = defaultStyleOrder.map((fallbackStyle, index) => requestedStylePresets[index] || fallbackStyle);

  const variants = [primary];

  variants[0] = {
    ...variants[0],
    stylePreset: String(finalVariantStyles[0] || payload.stylePreset || "balanced"),
    variantStyleLabel: getVariantStyleLabel(finalVariantStyles[0] || payload.stylePreset || "balanced", payload.lang)
  };

  for (let index = 1; index < variantCount; index += 1) {
    const styleKey = finalVariantStyles[index] || "balanced";
    const styleMap = {
      balanced: { tone: 0, brandStyle: 0, mood: 0 },
      expert: { tone: 1, brandStyle: 2, mood: 3 },
      sales: { tone: 2, brandStyle: 1, mood: 3 },
      lifestyle: { tone: 0, brandStyle: 1, mood: 1 },
      storytelling: { tone: 1, brandStyle: 2, mood: 1 },
      socialproof: { tone: 1, brandStyle: 2, mood: 2 },
      comparison: { tone: 0, brandStyle: 1, mood: 2 },
      benefitstack: { tone: 2, brandStyle: 2, mood: 3 },
      problemfirst: { tone: 0, brandStyle: 2, mood: 3 },
      premium: { tone: 1, brandStyle: 0, mood: 0 },
      urgencysoft: { tone: 2, brandStyle: 1, mood: 2 },
      educational: { tone: 1, brandStyle: 0, mood: 2 },
      community: { tone: 0, brandStyle: 3, mood: 1 },
      minimalist: { tone: 0, brandStyle: 0, mood: 1 },
      custom: {
        tone: Number.isFinite(Number(payload.tone)) ? Number(payload.tone) : 0,
        brandStyle: Number.isFinite(Number(payload.brandStyle)) ? Number(payload.brandStyle) : 0,
        mood: Number.isFinite(Number(payload.mood)) ? Number(payload.mood) : 0
      }
    };

    const style = styleMap[styleKey] || styleMap.balanced;
    const variantResult = await generateProductCopy({
      ...payload,
      improved: false,
      previousResult: null,
      tone: style.tone,
      ...
      variantStyleLabel: getVariantStyleLabel(styleKey, payload.lang)
    });
  ...
  let bestScore = variants[0]?.quality?.score || 0;
  for (let index = 1; index < variants.length; index += 1) {
    const score = variants[index]?.quality?.score || 0;
    if (score > bestScore) {
      bestScore = score;
...
export async function suggestProductFromImages(payload) {
  const outputConfig = getOutputLanguageConfig(payload?.lang);
  const outputLanguageLabel = `${outputConfig.labelEn} (${outputConfig.code})`;
  const imageQuality = getImageQualitySummary(payload?.images || []);
  const baseSearchFirst = buildSearchFirstClassifier(payload, imageQuality);
  const fallback = {
    category: "other",
    tone: 0,
    channel: 2,
  ...
  fallback.facts = mergeSearchFirstFacts(buildFactsFromNameAndCategory({
    productName: payload?.productName || "",
    category: baseSearchFirst.category || "other",
    lang: payload?.lang || "vi"
  }), baseSearchFirst);

  const images = Array.isArray(payload?.images) ? payload.images.slice(0, 4) : [];
  if (!images.length || !imageQuality.hasEnoughSignal) {
    trackAiUsageEvent({ type: "suggest_fallback" });
    const isVi = getLangKey(payload?.lang) === "vi";
    const qualityNotes = [];
    if (!imageQuality.total) {
      qualityNotes.push(outputConfig.suggestNoDataNote);
    } else {
      qualityNotes.push(
        isVi
          ? "Ảnh tải lên chưa đủ rõ để AI phân tích ổn định."
          : "Uploaded images are not clear enough for stable AI analysis."
      );
      if (imageQuality.lowQualityCount > 0) {
        qualityNotes.push(
          isVi
            ? "Nên dùng ảnh chụp thật sản phẩm, chủ thể chiếm phần lớn khung hình, hạn chế screenshot."
            : "Use real product photos where the product fills most of the frame, avoid screenshots."
        );
      }
      qualityNotes.push(
        isVi
          ? "Ảnh cần đủ nét và sáng (tránh rung/mờ), ưu tiên chụp gần sản phẩm."
          : "Ensure the photo is sharp and well lit (avoid blur), and capture the product closer."
      );
    }

    const nameFallback = buildNameOnlySuggestion({
      ...payload,
      productName: payload?.productName || ""
    }, outputConfig);
    const mergedFacts = mergeSearchFirstFacts(nameFallback.facts || {}, baseSearchFirst);
    const searchNote = getSearchFirstNote(baseSearchFirst, payload?.lang || "vi");
    return {
      ...fallback,
      category: baseSearchFirst.category !== "other" ? baseSearchFirst.category : fallback.category,
      generatedProductName: compact(nameFallback.generatedProductName || fallback.generatedProductName || ""),
      facts: mergedFacts,
      confidence: Math.max(0.2, fallback.confidence || 0.2),
      notes: [
        ...qualityNotes,
        searchNote
  ...
  const visualClassifyPrompt = `You are a strict visual product classifier.
Analyze ONLY the uploaded product images and return valid JSON only.

Schema:
{
  "objectType":"short noun phrase",
  "categoryHint":"fashion|skincare|beautyTools|home|furnitureDecor|electronics|food|householdEssentials|footwear|bags|accessories|fragrance|pet|sports|motherBaby|healthCare|booksStationery|toysGames|autoMoto|phoneTablet|computerOffice|cameraDrone|homeAppliances|toolsHardware|digitalGoods|other",
  "generatedProductNameHint":"...",
  "brandName":"",
  "modelName":"",
  "color":"",
...
Rules:
- Prioritize visual evidence from product pixels and visible packaging text.
- Do not infer from filename conventions.
- If brand/model/color/size is not clearly visible, return empty string.
...
- Prioritize visual evidence from clear product photos; if multiple images conflict, mention uncertainty in notes.
- Ignore screenshot UI/text overlays unless they describe the product itself.
...
- facts rules:
  - Extract brand/model/color/size/material only when there is visual or text evidence in image/name.
  - If uncertain, keep field empty string instead of guessing.
  - Put important factual lines into keySpecs and specialFeatures.
  - Populate facts.advanced based on category: electronics->specs/compatibility/warranty, home->dimensions/usageSpace/warranty, skincare->usage/skinConcern/routineStep, fashion->sizeGuide/careGuide/exchangePolicy.
...
- Image quality profile:
  - uploaded image count: ${imageQuality.total}
  - strong quality images: ${imageQuality.strongCount}
  - low quality/screenshot-like images: ${imageQuality.lowQualityCount}
Visual-first pre-classification signals (use as hints, not absolute truth):
- objectTypeHint: ${searchFirst.objectType || "N/A"}
- categoryHint: ${searchFirst.category || "other"}
- brandHint: ${searchFirst.brandName || "N/A"}
- modelHint: ${searchFirst.modelName || "N/A"}
- colorHint: ${searchFirst.color || "N/A"}
- sizeHint: ${searchFirst.sizeInfo || "N/A"}
- signalEvidence: ${(Array.isArray(searchFirst.evidence) && searchFirst.evidence.length) ? searchFirst.evidence.join(", ") : "N/A"}
- Important: prioritize visual/OCR evidence from uploaded images over text hints.
Product name: ${payload.productName || "N/A"}`;

  const applySearchFirstFallback = (suggestion = {}) => {
    const next = { ...(suggestion || {}) };
    const emptyUserName = compact(payload?.productName || "") === "";
    if (!next.category || next.category === "other") {
      if (searchFirst.category && searchFirst.category !== "other") {
        next.category = searchFirst.category;
      }
    }
    const normalizedHintName = normalizeGeneratedNameHint(searchFirst.generatedProductNameHint || "", payload?.lang || "vi");
    const composedSearchFirstName = composeGeneratedNameFromSearchFirst(searchFirst, payload?.lang || "vi");
    if (!compact(next.generatedProductName) || isUnknownGeneratedProductName(next.generatedProductName) || isGenericGeneratedProductName(next.generatedProductName)) {
      if (normalizedHintName) {
        next.generatedProductName = normalizedHintName;
      } else if (composedSearchFirstName) {
        next.generatedProductName = composedSearchFirstName;
      }
    }

    next.generatedProductName = normalizeGeneratedProductName(next.generatedProductName || "", payload?.lang || "vi");

    if (emptyUserName && isUnknownGeneratedProductName(next.generatedProductName || "")) {
      next.highlights = [];
      next.attributes = [];
    }
    const mergedFacts = mergeSearchFirstFacts(next.facts || {}, searchFirst);
    next.facts = mergedFacts;

    if (searchFirst.category && searchFirst.category !== "other" && Number(next.confidence || 0) < Number(searchFirst.confidence || 0)) {
      next.confidence = Number(searchFirst.confidence || next.confidence || 0.45);
    }

    const searchNote = getSearchFirstNote(searchFirst, payload?.lang || "vi");
    const visualNotes = asStringArray(searchFirst?.notes, { max: 2, maxLength: 120 });
    if (searchNote) {
      const notes = Array.isArray(next.notes) ? next.notes : [];
      if (!notes.some((item) => normalizeCompareText(item || "").includes("visual first"))) {
        next.notes = [searchNote, ...visualNotes, ...notes].slice(0, 4);
      }
    }
  ...
  let searchFirst = { ...baseSearchFirst };
  ...
  try {
    try {
      const visualResult = await requestWithVisionFallback(visualClassifyPrompt);
      const visualRaw = visualResult?.choices?.[0]?.message?.content;
      const visualParsed = parseJsonObject(extractMessageText(visualRaw));
      if (visualParsed && typeof visualParsed === "object") {
        const visualEvidence = normalizeVisualEvidence(visualParsed, payload?.lang || "vi");
        searchFirst = mergeSearchFirstWithVisualEvidence(searchFirst, visualEvidence);
      }
    } catch (visualError) {
      if (process.env.NODE_ENV !== "production") {
        console.warn("[AI][suggest] visual classify pass failed", visualError?.statusCode || "", visualError?.message || visualError);
      }
    }
    ...
    let suggestion = {
      ...
      attributes: Array.isArray(parsed.attributes)
        ? parsed.attributes
            ...
            .slice(0, 6)
        : [],
      facts: parseSuggestionFacts(parsed),
    ...
    suggestion = applySearchFirstFallback(suggestion);
    ...
    const nameFacts = buildFactsFromNameAndCategory({
      productName: payload?.productName || suggestion.generatedProductName || "",
      category: suggestion.category,
      lang: payload?.lang || "vi"
    });
    suggestion.facts = mergeSuggestionFacts(suggestion.facts, nameFacts);
    suggestion.facts = mergeSearchFirstFacts(suggestion.facts, searchFirst);
    enrichSuggestionWithFacts(suggestion, payload?.lang || "vi");

    if (!suggestion.notes.length) {
      suggestion.notes = [outputConfig.suggestNoDataNote];
    }

    if (hasNoDataSuggestionNote(suggestion.notes, outputConfig) && imageQuality.hasStrongSignal) {
      const strictRecoveryPrompt = `${basePrompt}\n\nRecovery pass rules:\n- You have at least one clear product image. Do not return generic insufficient-data notes.\n- Use the clearest image as primary evidence, and use product name as secondary signal.\n- Return practical category + prefill values with confidence >= 0.58 unless both signals are truly conflicting.\n- If confidence would be below 0.45, still provide best-effort broad category and practical prefill instead of no-data note.`;
      try {
        const recoveryResult = await requestWithVisionFallback(strictRecoveryPrompt);
        const recoveryRaw = recoveryResult.choices?.[0]?.message?.content;
        const recoveryParsed = parseJsonObject(extractMessageText(recoveryRaw));
        if (recoveryParsed && typeof recoveryParsed === "object") {
          let recoveredSuggestion = {
            category: normalizeCategoryValue(recoveryParsed.category || suggestion.category || "other"),
            tone: toNumberOr(recoveryParsed.tone, suggestion.tone),
            channel: toNumberOr(recoveryParsed.channel, suggestion.channel),
            mood: toNumberOr(recoveryParsed.mood, suggestion.mood),
            brandStyle: toNumberOr(recoveryParsed.brandStyle, suggestion.brandStyle),
            generatedProductName: normalizeGeneratedProductName(recoveryParsed.generatedProductName || recoveryParsed.inferredProductName || suggestion.generatedProductName || "", payload?.lang),
            targetCustomer: compact(recoveryParsed.targetCustomer || suggestion.targetCustomer || ""),
            shortDescription: compact(recoveryParsed.shortDescription || suggestion.shortDescription || ""),
            highlights: asStringArray(recoveryParsed.highlights, { max: 5, maxLength: 90 }),
            attributes: Array.isArray(recoveryParsed.attributes)
              ? recoveryParsed.attributes
                  .map((item, idx) => ({ type: toNumberOr(item?.type, idx), value: compact(item?.value || "") }))
                  .filter((item) => item.value)
                  .slice(0, 6)
              : suggestion.attributes,
            facts: parseSuggestionFacts(recoveryParsed, suggestion.facts || {}),
            confidence: Math.max(0, Math.min(1, Number(recoveryParsed.confidence) || suggestion.confidence || 0.45)),
            notes: asStringArray(recoveryParsed.notes, { max: 4, maxLength: 120 })
          };

          recoveredSuggestion = applySearchFirstFallback(recoveredSuggestion);

          const recoveredNameFacts = buildFactsFromNameAndCategory({
            productName: payload?.productName || recoveredSuggestion.generatedProductName || "",
            category: recoveredSuggestion.category,
            lang: payload?.lang || "vi"
          });
          recoveredSuggestion.facts = mergeSuggestionFacts(recoveredSuggestion.facts, recoveredNameFacts);
          recoveredSuggestion.facts = mergeSearchFirstFacts(recoveredSuggestion.facts, searchFirst);
          enrichSuggestionWithFacts(recoveredSuggestion, payload?.lang || "vi");

          if (!hasNoDataSuggestionNote(recoveredSuggestion.notes, outputConfig)) {
            suggestion.category = recoveredSuggestion.category;
            suggestion.tone = recoveredSuggestion.tone;
            suggestion.channel = recoveredSuggestion.channel;
            suggestion.mood = recoveredSuggestion.mood;
            suggestion.brandStyle = recoveredSuggestion.brandStyle;
            suggestion.generatedProductName = recoveredSuggestion.generatedProductName;
            suggestion.targetCustomer = recoveredSuggestion.targetCustomer;
            suggestion.shortDescription = recoveredSuggestion.shortDescription;
            suggestion.highlights = recoveredSuggestion.highlights;
            suggestion.attributes = recoveredSuggestion.attributes;
            suggestion.facts = recoveredSuggestion.facts;
            suggestion.confidence = recoveredSuggestion.confidence;
            suggestion.notes = recoveredSuggestion.notes;
          }
        }
      } catch (recoveryError) {
        if (process.env.NODE_ENV !== "production") {
          console.warn("[AI][suggest] recovery pass failed", recoveryError?.statusCode || "", recoveryError?.message || recoveryError);
        }
      }

      if (hasNoDataSuggestionNote(suggestion.notes, outputConfig)) {
        const repaired = repairWeakSuggestionWithNameContext(suggestion, payload, outputConfig, imageQuality);
        Object.assign(suggestion, repaired);
      }
    }
    ...
    if (isLikelyImageAnalysisFallback(suggestion, outputConfig)) {
      ...
      try {
        ...
        if (retryParsed && typeof retryParsed === "object") {
          let retrySuggestion = {
            ...
            attributes: Array.isArray(retryParsed.attributes)
              ? retryParsed.attributes
                  ...
                  .slice(0, 6)
              : suggestion.attributes,
            facts: parseSuggestionFacts(retryParsed, suggestion.facts || {}),
          ...
          retrySuggestion = applySearchFirstFallback(retrySuggestion);

          const retryNameFacts = buildFactsFromNameAndCategory({
            productName: payload?.productName || retrySuggestion.generatedProductName || "",
            category: retrySuggestion.category,
            lang: payload?.lang || "vi"
          });
          retrySuggestion.facts = mergeSuggestionFacts(retrySuggestion.facts, retryNameFacts);
          retrySuggestion.facts = mergeSearchFirstFacts(retrySuggestion.facts, searchFirst);
          enrichSuggestionWithFacts(retrySuggestion, payload?.lang || "vi");
          ...
          if (!isLikelyImageAnalysisFallback(retrySuggestion, outputConfig)) {
            ...
            return applySearchFirstFallback(retrySuggestion);
          }
        }
      } catch (retryError) {
        if (process.env.NODE_ENV !== "production") {
      ...
      const imageOnlyHeuristic = applySearchFirstFallback(buildImageOnlyHeuristicSuggestion(payload, outputConfig));
    ...
    trackAiUsageEvent({ type: "suggest_success" });
    return applySearchFirstFallback(suggestion);
  } catch (error) {
    if (process.env.NODE_ENV !== "production") {
    ...
    const nameOnly = applySearchFirstFallback(buildNameOnlySuggestion(payload, outputConfig));
    trackAiUsageEvent({ type: "suggest_fallback" });
    return nameOnly;
  }
}
