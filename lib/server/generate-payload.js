...
function sanitizePreviousResult(raw) {
  ...
  const hashtags = Array.isArray(raw.hashtags)
    ...
    : [];
  const variantGroupId = cleanText(raw.variantGroupId, 120);
  const variantStyleLabel = cleanText(raw.variantStyleLabel, 80);
  const stylePreset = cleanText(raw.stylePreset, 32).toLowerCase();
  const variantIndexRaw = Number(raw.variantIndex);
  const variantIndex = Number.isFinite(variantIndexRaw) && variantIndexRaw >= 0
    ? Math.floor(variantIndexRaw)
    : null;

  if (!paragraphs.length && !hashtags.length && !variantGroupId && !variantStyleLabel && !stylePreset && variantIndex === null) {
    return null;
  }
...
function sanitizeVariantStylePresets(raw, variantCount, fallbackStylePreset = "balanced") {
  const count = Math.max(1, Math.min(5, Number(variantCount) || 1));
  const allowed = new Set(["balanced", "expert", "sales", "lifestyle", "custom"]);
  const normalizedFallback = String(fallbackStylePreset || "balanced").toLowerCase().trim();
  const seed = allowed.has(normalizedFallback) ? normalizedFallback : "balanced";
  ...
  if (!cleaned.length) return [seed];

  const next = [];
  for (let index = 0; index < count; index += 1) {
    next.push(cleaned[index] || cleaned[0] || seed);
  }
  return next;
}
...
export function normalizeGeneratePayload(raw) {
  ...
  const variantCount = clampInt(raw.variantCount, 1, 5, 1);
  const brandPreset = cleanText(raw.brandPreset || "", 48).toLowerCase();
  const stylePreset = cleanText(raw.stylePreset || "", 32).toLowerCase();
  const variantStylePresets = sanitizeVariantStylePresets(raw.variantStylePresets, variantCount, stylePreset || "balanced");
