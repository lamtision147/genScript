...
function buildPrompt(payload) {
  ...
  const qualityBlock = langKey === "en"
      ? [
        ...
        "- If mentioning price, prefer scene 2 or scene 3 and keep it conversational.",
    ...
    : [
      ...
      "- Nếu nhắc giá, ưu tiên đưa ở cảnh 2 hoặc cảnh 3, nói tự nhiên như lời đời thường.",
...
function buildVnPriceAnalogy(priceLabel = "") {
  ...
  if (!text) return "";
  if (/45k|50k|4\s*[5-9]k|5\d\s*k/.test(text)) return "1 ly trà sữa";
  if (/1\d\d\s*k|2\d\d\s*k|3\d\d\s*k|duoi\s*200k|chua\s*den\s*200k/.test(text)) return "2-3 ly trà sữa";
  if (/500k|600k|700k|800k/.test(text)) return "một buổi cafe chill cuối tuần";
  if (/tri[eệ]u|\d\s*tr/.test(text)) return "một lần sắm đồ xịn cho bản thân";
  return "cỡ vài ly trà sữa";
}

function buildPlayfulPriceHint(payload = {}, langKey = "vi") {
  const hintRaw = normalizePriceSegmentHint(payload?.priceSegment || "", langKey);
  const hint = langKey === "vi" ? normalizeVnNumericPriceText(hintRaw) : hintRaw;
  if (!hint) return "";
  if (langKey !== "vi") {
  ...
  if (/(\d|k|tr|tri[eệ]u|vnd|đ)/i.test(hint)) {
    const analogy = buildVnPriceAnalogy(hint);
    return `Tầm ${hint}, tính ra cỡ ${analogy}, nên nghe xong thấy khá dễ chốt.`;
  }
  if (/de\s*chot|budget|thap/.test(hint)) {
    return "Giá mềm, kiểu vài ly trà sữa là có, nên mua thử cũng đỡ lăn tăn ví tiền.";
  }
  if (/cao|premium/.test(hint)) {
    return "Giá nhỉnh hơn xíu nhưng đổi lại trải nghiệm dùng lâu dài thấy đáng tiền.";
  }
  return `Giá tầm ${hint}, đủ hợp túi để cân nhắc chốt khá nhanh.`;
}
...
function injectPriceHintToVideoResult(result = {}, payload = {}) {
  ...
  const middleIndexes = [];
  if (scenes.length >= 2) middleIndexes.push(1);
  if (scenes.length >= 3) middleIndexes.push(2);
  const hasMiddlePriceMention = middleIndexes.some((index) => hasPriceMention(scenes[index]?.voice || ""));
  if (hasMiddlePriceMention) return result;

  const priceHint = buildPlayfulPriceHint(payload, langKey);
  if (!priceHint) return result;

  const targetIndex = middleIndexes.length ? middleIndexes[middleIndexes.length - 1] : Math.max(0, scenes.length - 1);
  const targetScene = scenes[targetIndex] || {};
  const baseVoice = String(targetScene?.voice || "").trim();
  let mergedVoice = compact([baseVoice, priceHint].filter(Boolean).join(" "), 260);

  if (langKey === "vi") {
    mergedVoice = mergedVoice
      .replace(/\bMức\s*giá\b/gi, "Tầm giá")
      .replace(/\bGiá\s*vibe\b/gi, "Giá")
      .replace(/\bnghe\s*phát\s*là\s*thấy\b/gi, "nghe xong thấy")
      .replace(/\s{2,}/g, " ")
      .trim();
  }
