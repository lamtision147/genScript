...
function toVariantGroupDateSuffix() {
  const d = new Date();
...
export async function listHistoryByVariantGroupAsync(userId, variantGroupId, options = {}) {
  ...
  if (supabase) {
    ...
    if (supabaseUserId) {
      const { data, error } = await supabase
        ...
        .select("*")
        .eq("user_id", supabaseUserId)
        .contains("form_data", { variantGroupId: normalizedGroupId })
        .order("created_at", { ascending: false });

      if (!error && Array.isArray(data)) {
        const normalized = data.map(normalizeHistoryRecord);
        return filterHistoryByType(normalized, options.type);
      }
    }
  }

  const localRows = filterHistoryByOwner(readJsonArray(paths.history), userId)
    .filter((item) => String(item?.form?.variantGroupId || "") === normalizedGroupId)
    .sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || "")));
  return filterHistoryByType(localRows, options.type);
}
...
export async function createHistoryItemAsync({ userId, title, variantLabel, formData, resultData, images = [] }) {
  const normalizedVariantGroupId = String(formData?.variantGroupId || "").trim();
  const shouldUpdateByGroup = Boolean(normalizedVariantGroupId) && /_improved_/.test(normalizedVariantGroupId);

  if (shouldUpdateByGroup) {
    const existingGroupItems = await listHistoryByVariantGroupAsync(userId, normalizedVariantGroupId, {
      type: formData?.contentType || "product_copy"
    });

    if (existingGroupItems.length) {
      const variantIndex = Number(resultData?.variantIndex || 0);
      const matched = existingGroupItems.find((item) => Number(item?.result?.variantIndex || 0) === variantIndex)
        || existingGroupItems[Math.max(0, Math.min(existingGroupItems.length - 1, variantIndex))]
        || existingGroupItems[0];

      if (matched?.id) {
        return updateHistoryItemOutputByUserAsync(userId, matched.id, {
          title,
          resultData
        });
      }
  ...
  if (supabase && userId) {
    ...
    try {
      if (supabaseUserId) {
        const insertPayload = createSupabaseHistoryInsert({ userId: supabaseUserId, title, variantLabel, formData, resultData, images });
        const { data } = await supabase
          .from("history_items")
...
export function buildImprovedVariantGroupId(previousGroupId = "") {
  const normalized = String(previousGroupId || "").trim();
  if (!normalized) {
    return `group_improved_${toVariantGroupDateSuffix()}_${Math.random().toString(36).slice(2, 8)}`;
  }
  if (/_improved_/.test(normalized)) {
    return normalized;
  }
  return `${normalized}_improved_${toVariantGroupDateSuffix()}`;
}
