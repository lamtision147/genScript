...
const OTP_STORE_PATH = paths.authOtp;
const OTP_STORE_CLEANUP_INTERVAL_MS = 60 * 1000;
const OTP_STORE_MAX_ITEMS = 500;

let otpStoreHydrated = false;
let otpStoreLastWriteAt = 0;
let otpStoreDirty = false;
let otpStoreFlushTimer = null;
let otpStoreCleanupTimer = null;
...
function safeSetInterval(handler, ms) {
...
function readOtpPersistenceFile() {
  const rows = readJsonArray(OTP_STORE_PATH);
  if (!Array.isArray(rows)) return [];
  return rows;
}

function flushOtpStoresToDiskImmediate() {
  const rows = [];
  const now = Date.now();
  ...
  for (const [email, value] of passwordResetStore.entries()) {
    if (!value || Number(value.expiresAt) <= now) continue;
    rows.push({
      kind: "password_reset",
      email,
  ...
  writeJsonArray(OTP_STORE_PATH, rows.slice(0, OTP_STORE_MAX_ITEMS));
  otpStoreLastWriteAt = now;
  otpStoreDirty = false;
}

function flushOtpStoresSoon() {
  otpStoreDirty = true;
  if (otpStoreFlushTimer) return;
  otpStoreFlushTimer = setTimeout(() => {
    otpStoreFlushTimer = null;
    if (!otpStoreDirty) return;
    flushOtpStoresToDiskImmediate();
  }, 200);
  if (typeof otpStoreFlushTimer?.unref === "function") {
    otpStoreFlushTimer.unref();
  }
}

function trimMapToLimitByExpiry(store, maxItems) {
  if (store.size <= maxItems) return;
  const sorted = [...store.entries()].sort((a, b) => Number(a[1]?.expiresAt || 0) - Number(b[1]?.expiresAt || 0));
  const toRemove = sorted.slice(0, Math.max(0, sorted.length - maxItems));
  for (const [key] of toRemove) {
    store.delete(key);
  }
}

function cleanupExpiredOtpEntries() {
  const now = Date.now();
  let changed = false;
  ...
  trimMapToLimitByExpiry(otpStore, OTP_STORE_MAX_ITEMS);
  trimMapToLimitByExpiry(passwordResetStore, OTP_STORE_MAX_ITEMS);
...
function ensureOtpStoresHydrated() {
  if (otpStoreHydrated) return;
  otpStoreHydrated = true;

  const rows = readOtpPersistenceFile();
  const now = Date.now();
  for (const row of rows) {
    const email = String(row?.email || "").trim().toLowerCase();
    const code = String(row?.code || "").trim();
    const expiresAt = Number(row?.expiresAt || 0);
    if (!email || !code || !Number.isFinite(expiresAt) || expiresAt <= now) continue;
  ...
  cleanupExpiredOtpEntries();

  if (!otpStoreCleanupTimer) {
    otpStoreCleanupTimer = safeSetInterval(() => {
      cleanupExpiredOtpEntries();
    }, OTP_STORE_CLEANUP_INTERVAL_MS);
  }
}
...
export function getNoReplyFromAddress() {
  const configured = String(process.env.MAIL_FROM_NO_REPLY || process.env.SMTP_FROM_NO_REPLY || "").trim();
  if (configured) return configured;

  const configuredDomain = String(process.env.MAIL_FROM_DOMAIN || "").trim().toLowerCase();
  if (configuredDomain) {
    return `Seller Studio <no-reply@${configuredDomain}>`;
  }

  const smtpUser = String(process.env.SMTP_USER || "").trim().toLowerCase();
  const inferredDomain = smtpUser.includes("@") ? smtpUser.split("@").pop() : "sellerstudio.local";
  return `Seller Studio <no-reply@${inferredDomain}>`;
}
...
export async function sendRenewalReminderEmail({ email, name, expiresAt, remainingDays, upgradeUrl }) {
  const transport = getMailTransport();
  if (!transport) return false;

  const safeName = String(name || "").trim() || "ban";
  const expiryDate = String(expiresAt || "").trim();
  const days = Number.isFinite(Number(remainingDays)) ? Math.max(0, Number(remainingDays)) : null;
  const subject = days !== null && days <= 1
    ? "Seller Studio - Pro sap het han trong 1 ngay"
    : "Seller Studio - Nhac gia han goi Pro";

  const textLines = [
    `Chao ${safeName},`,
    "",
    "Goi Pro cua ban sap het han.",
    days !== null ? `Con lai: ${days} ngay.` : "",
    expiryDate ? `Ngay het han: ${expiryDate}` : "",
    upgradeUrl ? `Gia han tai: ${upgradeUrl}` : "",
    "",
    "Neu ban da gia han, vui long bo qua email nay.",
    "- Seller Studio"
  ].filter(Boolean);

  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.55;color:#25180d">
      <h2 style="margin:0 0 12px">Seller Studio</h2>
      <p>Chao ${safeName},</p>
      <p>Goi <strong>Pro</strong> cua ban sap het han.</p>
      ${days !== null ? `<p><strong>Con lai:</strong> ${days} ngay</p>` : ""}
      ${expiryDate ? `<p><strong>Ngay het han:</strong> ${expiryDate}</p>` : ""}
      ${upgradeUrl ? `<p><a href="${upgradeUrl}" style="display:inline-block;padding:10px 16px;background:#ff8a1f;color:#fff;text-decoration:none;border-radius:8px">Gia han goi Pro</a></p>` : ""}
      <p style="color:#5a4738">Neu ban da gia han, vui long bo qua email nay.</p>
    </div>
  `;
